...
for trg_lang in trg_langs:
    with open(os.path.join(data_dir, "devtest.eng_Latn"), "r") as f_src, open(
        os.path.join(data_dir, f"devtest.{trg_lang}"), "r"
    ) as f_trg:
...
df_ref = pd.DataFrame(
    {"source": src_data, "target": trg_data, "trg_lang": trg_lang_data}
)
...
project.upload_dataset(
    df_ref, id_column="id", data_column="source", label_column="target"
)
...
def calculate_chrf(predictions, references):
    return [
        chrf_evaluator.compute(predictions=[pred], references=[[ref]])["score"]
...
for trg_lang in trg_langs:
    for system_name in os.listdir(system_outputs_dir):
        ...
        project.upload_system(
            df_system, name=system_name, id_column="id", output_column="output"
        )
